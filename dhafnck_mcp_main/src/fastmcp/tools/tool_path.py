...
def find_project_root(start_path: Path = None) -> Path:
    ...
    for a marker file (.git first, then pyproject.toml, or cursor_agent subdirectory). Returns a pathlib.Path to the project root.
    ...
    # First, search for .git directory (highest priority for true project root)
    for parent in [current] + list(current.parents):
        if (parent / ".git").exists():
            return parent
    
    # If no .git found, search for other markers
    for parent in [current] + list(current.parents):
        if (parent / "pyproject.toml").exists() or (parent / "cursor_agent").is_dir():
            return parent
