    ...
    def execute(self, name_agent: str) -> Dict[str, Any]:
        ...
        try:
                ...
                for file in files:
                    if file.endswith(".yaml"):
                        ...
                        if isinstance(yaml_content, dict):
                            ...
                            if (
                                len(yaml_content) == 1
                                and file_name_without_ext in yaml_content
                            ):
                                ...
                                if isinstance(inner_content, dict):
                                    ...
                                    combined_content[file_name_without_ext] = (
                                        inner_content
                                    )
                            else:
            ...
            try:
                ...
                logging.warning(
                    f"Failed to generate agent documentation for {name_agent}: {str(e)}"
                )
                # Don't fail the entire operation if MDC generation fails

            return {"success": True, "agent_info": combined_content}
