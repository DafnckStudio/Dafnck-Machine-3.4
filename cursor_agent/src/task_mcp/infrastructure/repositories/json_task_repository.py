...
from ...domain import Priority, Task, TaskId, TaskRepository, TaskStatus
...
class InMemoryTaskRepository(TaskRepository):
    ...
    def find_by_criteria(
        ...
        for task in tasks:
            ...
            if "assignees" in criteria:
                ...
                if not any(
                    assignee in task.assignees for assignee in required_assignees
                ):
                    matches = False
    ...
    def find_by_labels(self, labels: List[str]) -> List[Task]:
        ...
        return [
            ...
            if all(label in task.labels for label in labels)
        ]
...
class JsonTaskRepository(TaskRepository):
    ...
    def __init__(self, file_path: Optional[str] = None):
        if file_path:
            ...
            # Navigate up to project root (agentic-project) and then down to .cursor/rules/tasks/tasks.json
            self._file_path = str(
    ...
    def _create_backup(self):
        ...
        if os.path.exists(self._file_path):
            ...
            backup_file = os.path.join(
                self._backup_path, f"tasks_backup_{timestamp}.json"
            )
        ...
        for task_dict in data.get("tasks", []):
            if task_dict["id"] == str(task_id):
                try:
                    ...
                    logging.error(
                        f"Error converting task dict to domain: {e} - Task data: {task_dict}"
                    )
                    continue
        return None
