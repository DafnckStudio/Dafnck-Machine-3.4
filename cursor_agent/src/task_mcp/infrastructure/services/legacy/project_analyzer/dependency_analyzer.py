...
class DependencyAnalyzer:
    ...
    def analyze_dependencies(self) -> List[str]:
        ...
        has_python_files = (
            any(cursor_agent_dir.glob("**/*.py"))
            if cursor_agent_dir.exists()
            else False
        )
        has_requirements_txt = (cursor_agent_dir / "requirements.txt").exists() or (
                    ...
                    with open(self.project_root / "package.json") as f:
                        ...
                        if "dependencies" in package_data:
                            js_deps.extend(
                                [
                                    f"{k} (JS)"
                                    for k in package_data["dependencies"].keys()
                                ]
                            )
                        if "devDependencies" in package_data:
                            js_deps.extend(
                                [
                                    f"{k} (JS dev)"
                                    for k in package_data["devDependencies"].keys()
                                ]
                            )
    ...
    def _categorize_imports(self, imports: List[str]) -> List[str]:
        ...
        if stdlib_found:
            stdlib_summary = (
                f"Python Standard Library ({', '.join(sorted(stdlib_found)[:10])})"
            )
