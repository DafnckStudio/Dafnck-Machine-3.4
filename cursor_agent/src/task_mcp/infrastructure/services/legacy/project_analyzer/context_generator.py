...
class ContextGenerator:
    ...
    def _get_phase_specific_context(self, phase: str, patterns: List[str]) -> str:
        ...
        # Add base guidance
        phase_guidance = base_guidance.get(
            phase, [f"Focus on {phase} activities according to project patterns"]
        )
    ...
    def _get_pattern_specific_guidance(
        self, phase: str, patterns: List[str]
    ) -> List[str]:
        ...
        if any("Python" in pattern for pattern in patterns):
            if phase == "coding":
                ...
                guidance.append(
                    "- Use pytest or unittest following project conventions"
                )
        ...
        if any("Dataclass" in pattern for pattern in patterns):
            if phase == "coding":
                guidance.append(
                    "- Use dataclasses for data models following existing patterns"
                )
        ...
        if any("MCP" in pattern for pattern in patterns):
            if phase in ["planning", "coding"]:
                guidance.append(
                    "- Follow MCP (Model Context Protocol) patterns and conventions"
                )
                            ...
                            if child_line.strip():  # Skip empty lines
                                ...
                                if level == 0:
                                    ...
                                    if is_last:
                                        ...
                                        adjusted_line = child_line.replace(
                                            "│   " * level, "    " * level, 1
                                        )
                                    else:
