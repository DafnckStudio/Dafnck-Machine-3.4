...
class RulesTemplateSystem:
    ...
    def generate_rules_content(
        self, task: TaskContext, role: AgentRole, project_context: Dict
    ) -> str:
        ...
        variables = self._build_template_variables(
            task, role, project_context, agent_data
        )
    ...
    def _build_template_variables(
        ...
        # Extract custom instructions from contexts (for potential future use)
        ...
        for content_type, builder in self.content_builders.items():
            variables[content_type] = builder(
                task, role, project_context, project_root, agent_data
            )
            ...
            if isinstance(rule_content, dict):
                for key, value in rule_content.items():
                    if isinstance(value, dict) and "mechanism" in value:
                        rules.append(
                            f"{key.replace('_', ' ').title()}: {value['mechanism']}"
                        )
    ...
    def _extract_context_instructions_from_agent_data(
    ...
    def _build_context_instructions_content(
        self,
        task: TaskContext,
        role: AgentRole,
        project_context: Dict,
        ...
        if agent_data:
            instructions = self._extract_context_instructions_from_agent_data(
                agent_data
            )
        else:
            instructions = (
                role.context_instructions if role.context_instructions else []
            )
    ...
    def _build_tools_guidance_content(
        self,
        task: TaskContext,
        role: AgentRole,
        project_context: Dict,
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                if self._is_special_section(key):
                    ...
                    formatted_section = self._format_yaml_data_structure(
                        value, level + 1
                    )
    ...
    def _format_complexity_table(self, data: Dict[str, Any]) -> str:
        ...
        for complexity in complexity_order:
            if complexity in data:
                ...
                content.append(
                    f"| **{complexity.title()}** | {duration} | {criteria} |"
                )
    ...
    def _format_phases_details(self, data: Dict[str, Any]) -> str:
        ...
        for phase_num in sorted(
            data.keys(), key=lambda x: int(x) if x.isdigit() else 999
        ):
    ...
    def _format_guidance_item(self, item: str) -> str:
        ...
        if "**" in cleaned_item and ":" in cleaned_item:
            ...
            "(" in cleaned_item
            and ")" in cleaned_item
            and any(word in cleaned_item.lower() for word in ["hour", "day", "week"])
        ):
            # Time estimate
            return f"- {cleaned_item}"
        else:
    ...
    def _build_phase_specific_content(
        self,
        task: TaskContext,
        role: AgentRole,
        project_context: Dict,
        ...
        if agent_data and agent_data.get("tools", {}):
            for item in agent_data["tools"].values():
                item_lower = item.lower()
                if (
                    task.current_phase.lower() in item_lower
                    or self._is_phase_relevant_content(item, task.current_phase)
                ):
        ...
        if agent_data and agent_data.get("rules", {}):
            for rule in agent_data["rules"].values():
                if isinstance(rule, str):
                    rule_lower = rule.lower()
                    if (
                        task.current_phase.lower() in rule_lower
                        or self._is_phase_relevant_content(rule, task.current_phase)
                    ):
        ...
        if phase_specific_items:
            ...
            categorized_items = self._categorize_phase_content(
                phase_specific_items, task.current_phase
            )
            ...
            phase_content.append(f"### {task.current_phase.title()} Phase Focus")
            generic_guidance = self._get_generic_phase_guidance(
                task.current_phase, role
            )
    ...
    def _categorize_phase_content(
        self, items: List[str], phase: str
    ) -> Dict[str, List[str]]:
        ...
        for item in items:
            ...
            if any(
                ...
                for keyword in ["tool", "framework", "automation", "use", "implement"]
            ):
            ...
            # Check for deliverables
                ...
                keyword in item_lower
                for keyword in ["create", "generate", "produce", "deliver", "output"]
            ):
    ...
    def _get_generic_phase_guidance(self, phase: str, role: AgentRole) -> List[str]:
        ...
        return generic_guidance.get(
            phase.lower(),
            [f"- Focus on {phase} activities according to role responsibilities"],
        )
    ...
    def _build_project_context_content(
        ...
        if patterns:
            if len(patterns) > 8:
                ...
                patterns_content = "\n".join(
                    [f"- {pattern}" for pattern in patterns[:8]]
                )
...
"""
    ...
    def _build_all_roles_info_content(
        self,
        task: TaskContext,
        role: AgentRole,
        project_context: Dict,
    ...
    def _build_context_link_content(
        self,
        task: TaskContext,
        role: AgentRole,
        project_context: Dict,
