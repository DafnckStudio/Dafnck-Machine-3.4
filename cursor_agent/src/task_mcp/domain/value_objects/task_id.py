...
class TaskId:
    ...
    def __post_init__(self):
        ...
        if not self._is_valid_format(value_str):
            raise ValueError(
                f"Task ID must be in YYYYMMDDXXX or YYYYMMDDXXX.XXX format, got: {value_str}"
            )
    ...
    def generate_new(cls, existing_ids: list = None) -> "TaskId":
        ...
        for task_id in existing_ids:
            try:
                tid = cls(str(task_id))
                if tid.date_part == current_date and not tid.is_subtask:
                    sequence_num = int(tid.sequence_part)
        ...
        if next_sequence > 999:
            raise ValueError(
                f"Maximum tasks per day (999) exceeded for date {current_date}"
            )

        return cls(f"{current_date}{next_sequence:03d}")

    @classmethod
    def generate_subtask(
        cls, parent_id: "TaskId", existing_subtask_ids: list = None
    ) -> "TaskId":
        ...
        if next_sequence > 999:
            raise ValueError(
                f"Maximum subtasks per task (999) exceeded for task {parent_id}"
            )
