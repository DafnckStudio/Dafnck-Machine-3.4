...
def _get_project_root() -> Path:
    ...
    # Go up until we find the directory containing cursor_agent, or are in cursor_agent
    ...
    raise FileNotFoundError(
        "Could not find project root containing 'cursor_agent' directory."
    )
...
class CursorRulesTools:
    ...
    def get_all_tools(self) -> list:
        ...
        return [
            self.update_auto_rule,
            self.validate_rules,
            self.manage_cursor_rules,
            self.regenerate_auto_rule,
            self.validate_tasks_json,
        ]

    def register_tools(self, mcp):
        """Register all cursor rules tools with the MCP server"""
        mcp.tool(self.update_auto_rule)
        mcp.tool(self.validate_rules)
        mcp.tool(self.manage_cursor_rules)
        mcp.tool(self.regenerate_auto_rule)
        mcp.tool(self.validate_tasks_json)

    def update_auto_rule(self, content: str, backup: bool = True) -> Dict[str, Any]:
        ...
        try:
            auto_rule_path = (
                self._project_root / ".cursor" / "rules" / "auto_rule.mdc"
            )
    ...
    def validate_rules(self, file_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    def manage_cursor_rules(
        self, action: str, target: Optional[str] = None, content: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def regenerate_auto_rule(
        self, role: Optional[str] = None, task_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Regeneration failed: {str(e)}"}

    def validate_tasks_json(
        self, file_path: Optional[str] = None, output_format: str = "summary"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "tasks_validator", validator_path
            )
            ...
            if output_format == "json":
                ...
                return {
                    ...
                    "critical_errors": (
                        result["errors"][:3] if result["errors"] else []
                    ),
                    "recommendations": (
